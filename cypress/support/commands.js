...
Cypress.Commands.add('login', (
    ...
    password = Cypress.env('USER_PASSWORD'), //.env é o valor default, mas passando na spec, pode ser alterado para um novo valor
    { cacheSession = true } = {} //
) => {
    const login = () => {
        cy.visit('/login')
        cy.get('#email').type(username) //Passando apenas o parâmetro, já que foi definido um default
        cy.get('#password').type(password, { log: false })
